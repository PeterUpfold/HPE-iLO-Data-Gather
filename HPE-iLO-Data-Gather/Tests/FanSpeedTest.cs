        ...
        public bool TestFanSpeed()
        {
            ...
            for (int i = 1; i <= 6; i++)
            {
                ...
                if (int.TryParse(fanCell.Text.Trim('%'), out int fanSpeedInt)) {
                    if (fanSpeedInt > (int)Settings.Default["FailTestsAtFanPercentage"])
                    {
                        Debug.WriteLine($"Fan {i} is running at {fanSpeedInt} (> threshold {Settings.Default["FailTestsAtFanPercentage"]}), so the test should fail");
                        shouldFailTest = true;
                    }
                }
